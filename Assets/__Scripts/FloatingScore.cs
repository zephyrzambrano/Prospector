...
public class FloatingScore : MonoBehaviour {
    ...
    // Note the use of parameter defaults for ETimS & eTimD
    public void Init(List<Vector2> ePts, float eTimS = 0, float eTimD = 1) {
        ...
        // If eTimS is the default, just start at the current time
        if (eTimS == 0) eTimS = Time.time;
        timeStart = eTimS;
        timeDuration = eTimD;
    ...
    void Update() {
        ...
        if (u < 0) { // If u < 0, then we shouldn't move yet
            ...
            if (u >= 1) { // If u >= 1, we're done moving
                ...
                state = eFSState.post;
                if (reportFinishTo != null) { // If there's a callback GameOject
